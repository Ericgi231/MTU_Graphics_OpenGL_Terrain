 ...
 *
 * - If ImageMagick is available: We ask ImageMagick to render the font
 *   using a normal font
 * 
 * - If ImageMagick is not present: We use a hardcoded 8x8 monospace
 *   bitmap font. The pointsize argument is ignored in this case..
 *
...
{
	// Use 8x8 bitmap rendering since making labels with imagemagick
	// is problematic (imagemagick 6 and 7 behave differently). Also,
	// ImageMagick is not a required library for opengl-examples and
	// using the bitmap version makes the label appear the same
	// regardless of if ImageMagick is installed or not.
